			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (test_bit(IMA_NS_DISABLED, &ns->ima_ns_flags))
	    v = 2;
	else if (test_bit(IMA_NS_ACTIVE, &ns->ima_ns_flags))
	    v = 1;
				...
				size_t count, loff_t *ppos)
{
	...
	/* accepting '<digit>\n' and '<digit>\0' and no partial writes */
	if (count >= 3 || *ppos != 0)
		return -EINVAL;
	...
	case 1:
		...
		break;
	case 2:
		set_bit(IMA_NS_DISABLED, &ns->ima_ns_flags);
		break;
	default:
		return -EINVAL;
	}
