				 ...
				 enum ima_hooks func, const uuid_t *src_userns)
{
	...
	if (backing_inode != inode &&
	    (action & IMA_DO_MASK) && (flags & IMA_DONE_MASK)) {
			 ...
			 unsigned long prot, unsigned long flags)
{
	...
	if (reqprot & PROT_EXEC) {
		ret = process_measurement(user_ns, file, current_cred(), secid,
					  NULL, 0, MAY_EXEC,
					  MMAP_CHECK_REQPROT);
		if (ret)
			return ret;
			     ...
			     unsigned long prot)
{
	...
	action = ima_get_action(ns, file_mnt_idmap(vma->vm_file), inode,
			       ...
			       uuid_t *src_userns)
{
	...
	if (func) {
		security_current_getsecid_subj(&secid);
		action = ima_get_action(ns, idmap, inode, current_cred(),
