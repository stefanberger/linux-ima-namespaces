...
/*
 * Check whether the verification material is prefixed by an OID of a hash
 * and followed by the exact number of bytes of the hash.
 */
static int _mldsa_vermat_has_oid(const uint8_t *vermat, unsigned int vermatlen)
{
	static const struct oid_hash {
		unsigned char oid_last;
		size_t	      digest_size;
	} oid_hash[3] = {
		{ .oid_last = 0x01, .digest_size = SHA256_DIGEST_SIZE },
		{ .oid_last = 0x03, .digest_size = SHA512_DIGEST_SIZE },
		{ .oid_last = 0x0b, .digest_size = 256 / 8 },
	};
	size_t i;
	...
	if (vermatlen < 11)
		return 0;
	if (memcmp(vermat,
	           (uint8_t[10]){0x06, 0x09, 0x60, 0x86, 0x48,
				 0x01, 0x65, 0x03, 0x04, 0x02},
		   10) != 0)
		return 0;

	for (i = 0; i < ARRAY_SIZE(oid_hash); i++) {
		if (vermat[10] != oid_hash[i].oid_last)
			continue;
		if (vermatlen - 11 == oid_hash[i].digest_size)
			return 1;
		break;
	}
	return 0;
 ...
 * @vermat: verification material such as plain message or digest with
 *          prepended OID for SHA256/512 or Shake128
 * @vermatlen: length of verification material
 */
static int mldsa_verify(struct crypto_sig *tfm,
			const void *src, unsigned int slen,
			const void *vermat, unsigned int vermatlen)
{
	const uint8_t mldsa_ds_prehash[2] = { 0x1, 0x0 };
	const uint8_t mldsa_ds_pure[2] = { 0x0, 0x0 };
	struct dil_ctx *ctx = crypto_sig_ctx(tfm);
	size_t domseplen = sizeof(mldsa_ds_pure);
	const uint8_t *domsep = mldsa_ds_pure;
	int ret;

	if (unlikely(ctx->pub_key_size) == 0)
		return -EKEYREJECTED;

	if (_mldsa_vermat_has_oid(vermat, vermatlen))
		domsep = mldsa_ds_prehash;

	ret = mldsa_verify_internal(src, slen, vermat, vermatlen, ctx->pub_key,
				    ctx->pub_key_size, domsep, domseplen);
