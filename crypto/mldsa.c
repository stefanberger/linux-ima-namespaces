...
#define MLCA_ALGORITHM_SIG_MLDSA_44_OID \
	"\x06\x09"                      \
	"\x60\x86\x48\x01\x65\x03\x04\x03\x11"
#define MLCA_ALGORITHM_SIG_MLDSA_65_OID \
	"\x06\x09"                      \
	"\x60\x86\x48\x01\x65\x03\x04\x03\x12"
#define MLCA_ALGORITHM_SIG_MLDSA_87_OID \
	"\x06\x09"                      \
	"\x60\x86\x48\x01\x65\x03\x04\x03\x13"

/*
 * Verify an ECDSA signature.
 */
static int mldsa_verify_sig(struct crypto_sig *tfm, const void *src,
			    ...
			    unsigned int dlen)
{
	...
	ret = mlca_verify(src, slen, digest, dlen, ctx->pub_key,
			  ctx->pub_key_size, oid, 11);
